...
- Get Flask version in `marrow_blog/page/views.py` with metadata instead of `__version__`
...
- In `marrow_blog/up/views.py`, update raw SQL `SELECT 1` health check for SQLAlchemy 2.0
...
- `marrow_blog/initializers.py` now exists to define frequently used imports, variables, etc.
...
- `marrow_blog/blueprints/` namespace has been removed
...
- Import `Redis` more efficiently in `marrow_blog/extensions.py`
...
- Unnecessary `CELERY_TASK_LIST` comment in `marrow_blog/app.py`, this was a left over copy / paste mishap
