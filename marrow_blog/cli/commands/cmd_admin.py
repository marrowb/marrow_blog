...
@click.option("--enable-mfa", is_flag=True, help="Enable MFA for the admin user.")
def create_admin(username, password, enable_mfa):
    """Creates or updates the admin user."""
    user = AdminUser.query.filter_by(username=username).first()
    if user:
        click.echo(f"Admin user '{username}' already exists. Updating password and MFA status.")
    else:
