...
def upgrade():
    ...
    op.add_column(
        "posts", sa.Column("slug", sa.String(length=255), nullable=True)
    )
    op.add_column("posts", sa.Column("excerpt", sa.Text(), nullable=True))
    op.add_column(
        "posts", sa.Column("tags", sa.String(length=500), nullable=True)
    )
    ...
    posts = connection.execute(
        sa.text("SELECT id, title, markdown_content FROM posts")
    ).fetchall()
    for post in posts:
        ...
        while connection.execute(
            sa.text("SELECT id FROM posts WHERE slug = :slug"), {"slug": slug}
        ).fetchone():
        ...
        connection.execute(
            sa.text(
                "UPDATE posts SET slug = :slug, excerpt = :excerpt WHERE id = :id"
            ),
    ...
    op.create_index(
        op.f("ix_posts_published"), "posts", ["published"], unique=False
    )
