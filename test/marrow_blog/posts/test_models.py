...
class TestPostModel(ViewTestMixin):
    ...
    def test_post_creation_with_required_fields(self, clean_session):
        ...
        post = Post(
            title="New Test Post", slug="new-test-post", author_id=admin.id
        )
    ...
    def test_post_save_method(self, clean_session):
        ...
        post = Post(title="Saved Post", slug="saved-post", author_id=admin.id)
    ...
    def test_post_delete_method(self, clean_session):
        ...
        post = Post(title="To Delete", slug="to-delete", author_id=admin.id)
        post.save()
...
class TestPostRelationships(ViewTestMixin):
    ...
    def test_post_author_foreign_key(self, clean_session):
        ...
        admin = AdminUser.query.filter_by(username="test_admin").first()
        post = Post(title="FK Test", slug="fk-test", author_id=admin.id)
        post.save()
...
class TestPostConstraints(ViewTestMixin):
    ...
    def test_title_uniqueness_constraint(self, clean_session):
        ...
        # Create first post
        post1 = Post(
            title="Unique Title", slug="unique-slug-1", author_id=admin.id
        )
        post1.save()
    ...
    def test_slug_uniqueness_constraint(self, clean_session):
        ...
        # Create first post
        post1 = Post(
            title="First Title", slug="unique-slug", author_id=admin.id
        )
        post1.save()
...
class TestResourceMixinIntegration(ViewTestMixin):
    ...
    def test_timestamps_auto_population(self, clean_session):
        ...
        post = Post(
            title="Timestamp Test", slug="timestamp-test", author_id=admin.id
        )
    ...
    def test_updated_on_changes_on_update(self, clean_session):
        ...
        post = Post(
            title="Update Test", slug="update-test", author_id=admin.id
        )
        post.save()
    ...
    def test_resource_mixin_str_method(self, clean_session):
        ...
        post = Post(title="Str Test", slug="str-test", author_id=admin.id)
        post.save()
    ...
    def test_timestamps_are_datetime_objects(self, clean_session):
        ...
        post = Post(
            title="DateTime Test", slug="datetime-test", author_id=admin.id
        )
        post.save()
        ...
        now = datetime.now()
        time_diff = (
            now - post.created_on.replace(tzinfo=None)
        ).total_seconds()
